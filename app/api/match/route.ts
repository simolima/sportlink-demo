...
export async function POST(req: Request) {
  ...
  const candidates = athletes.map((a: any) => { const r = score(a, need); return { name: a.profile.displayName, score: r.s, why: r.why }; })
    .sort((x: any, y: any) => y.score - x.score).slice(0, 10);
