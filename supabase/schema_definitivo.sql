...
('talent_scout', 'Talent Scout', 'Scopri e valuta nuovi talenti.', false, false, false, true, true);
...
-- 2.3 ORGANIZZAZIONI SPORTIVE (per autocomplete esperienze)
-- Database di tutte le società sportive (popolato da admin + richieste utenti)
...
-- 2.5 POSIZIONI SPECIFICHE (Dipendono da Sport e Ruolo)
...
-- 3.4 RICHIESTE ORGANIZZAZIONI (User-submitted organizations)
-- Gli utenti possono richiedere l'inserimento di società non presenti nel DB
-- NOTA: Spostata qui dopo profiles per risolvere dipendenze FK
create table public.organization_requests (
  ...
  requested_city text,
  requested_sport text not null,
  additional_info text,  -- Note aggiuntive dell'utente
  
  -- Metadata richiesta
  requested_by uuid references public.profiles(id) on delete cascade not null,
  requested_at timestamptz default now(),
  
  -- Stato della richiesta (gestito da admin)
  status text default 'pending' check (status in ('pending', 'approved', 'rejected', 'duplicate')),
  reviewed_by uuid references public.profiles(id),
  reviewed_at timestamptz,
  review_notes text,  -- Motivazione admin
  
  -- FK all'organizzazione creata (se approvata)
  created_organization_id uuid references public.sports_organizations(id),
  
  -- Evita spam: stesso utente non può richiedere 2 volte la stessa società
  constraint unique_request unique (requested_name, requested_country, requested_sport, requested_by)
);

-- Indici per dashboard admin
create index idx_org_requests_status on public.organization_requests (status, requested_at desc);
create index idx_org_requests_user on public.organization_requests (requested_by);
...
create table public.clubs (
  ...
  -- Collegamento all'organizzazione sportiva canonica
  -- Ogni club registrato DEVE avere un record in sports_organizations
  organization_id uuid references public.sports_organizations(id) on delete restrict not null unique,
...
-- 4.2 CLUB MEMBERSHIPS (Roster Attuale)
-- Chi fa parte del club ORA (admin, staff, giocatori attuali)
create table public.club_memberships (
  ...
  -- Unique: un utente può avere UN SOLO ruolo attivo per club
  unique(club_id, user_id, club_role)
);

-- 4.3 CAREER EXPERIENCES (CV Utente)
-- Storia completa della carriera (passate + presenti, club registrati + non registrati)
create table public.career_experiences (
  id uuid default gen_random_uuid() primary key,
  
  -- Organizzazione/Società sportiva (unica fonte di verità)
  organization_id uuid references public.sports_organizations(id) on delete cascade not null,
  
  user_id uuid references public.profiles(id) on delete cascade not null,
  
  -- Ruolo nell'esperienza
  role text not null check (role in ('Player', 'Coach', 'Staff', 'Other')),
  role_detail text,  -- Es: "Allenatore Primavera", "Preparatore Atletico"
  
  -- ====================================================================
  -- MODELLO STAGIONALE (Season-Based Experiences)
  -- ====================================================================
  
  -- Stagione sportiva (obbligatoria)
  season text not null,  -- Es: "2025/2026"
  
  -- Categoria competizione (obbligatoria da frontend)
  category text not null,  -- Es: "Serie A", "Serie B", "Primavera 1"
  
  -- Tipologia competizione per genere
  competition_type text not null check (competition_type in ('male', 'female', 'open', 'mixed')),
  
  -- Date specifiche (opzionali - per trasferimenti/prestiti mid-season)
  start_date date,  -- Nullable - es: "2025-09-01"
  end_date date,    -- Nullable - es: "2026-01-15"
  
  -- Flag esperienza in corso
  is_current boolean default false,
  
  -- ====================================================================
  -- STATISTICHE GIOCATORE (Player Stats)
  -- ====================================================================
  ...
  -- ====================================================================
  -- STATISTICHE ALLENATORE (Coach Stats)
  -- ====================================================================
  -- Valorizzate solo se role = 'Coach'
  ...
  -- Validazione coerenza statistiche allenatore
  constraint check_coach_stats_consistency 
    check (
      matches_coached is null 
      or matches_coached = 0 
      or matches_coached = (coalesce(wins, 0) + coalesce(draws, 0) + coalesce(losses, 0))
    ),
  ...
  -- Unique constraint: previene duplicati considerando stagione, categoria e ruolo
  -- Permette: stesso user, stessa organizzazione, stessa stagione MA categorie diverse (promozione)
  -- Permette: stesso user, stessa organizzazione, stessa stagione MA ruoli diversi (giocatore → allenatore)
  constraint unique_experience_per_season 
    unique(organization_id, user_id, season, category, role)
);
